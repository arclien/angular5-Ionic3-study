...
- 서비스는 비즈니스 로직을 담는다.
- 컴포넌트로부터 비즈니스 로직을 분리해서, 컴포넌트는 해당 뷰를 관리하고, 공용으로 사용되는 서비스의 로직을 통해 상호작용을 한다.
- 생성자를 선언 후, 컴포넌트에서 해당 서비스를 임포트하고, 생성자의 인자로 서비스를 추가하는데 이를 의존성 주입(dependency injection)이라고한다.
- 의존성 주입 정보는 앵귤러 모듈에 선언한다.(src/app/app.module.ts)

## 상태 관리 및 공유
- 뷰의 상태는 뷰와 연관된 컴포넌트가 관리. 그러나 여러 컴포넌트에 동일한 상태 정보가 공유되어야 할 경우도 있다.( 유저의 로그인 정보, 오늘 날짜 등등 )
- 여러 컴포넌트 간에 데이터를 서비스를 통해 공유를 하면 효과적이다. 각각의 컴포넌트에 저장했던 공유되어야할 정보를( 멤버변수 )를 서비스의 멤버변수로 저장해서, 서비스의 메서드를 통해 각 컴퍼넌트가 get/set을 통해 어플리케이션 전역을 통해 싱크를 맞춘다.

## directive(지시자)
- 지시자를 통해서 템플릿을 동적으로 만들고, 코드의 양을 줄일 수 있다.
- structural directive(구조 지시자)
    - DOM 구조를 동적으로 처리( *ngFor )
- attribute directive(속성 지시자)
    - DOM의 속성을 앵귤러 방식으로 관리

## Pipe(파이프)
- 템플릿에 데이터를 가공하여 보여줄 경우 사용.
- 예를들어 앵귤러에서 제공하는 Currency Pipel를 사용하면, 하나의 변수를 다양한 국가에 맞는 통화로 쉽게 출력할 수 있다.
```
price = 100;
{{ price | currency:'USD'}}
```
- 송아리앱에서는 나이를 바탕으로 다양하게 표현
    - birth = 1987의 값을 파이프를 통해, 87년생, 1987년, 32살 등등으로 파이프를 통해 쉽게 표현.

## Module(모듈)
- 모듈은 앵귤러에서 관련된 요소를 하나로 묶어 애플리케이션을 구성하는 단위.
- 앵귤러는 반드시 루트 모듈(AppModule)을 갖고 이 모듈을 통해 클래스를 생성한다.
- 위의 다양한 컴포넌트, 서비스, directive, 파이프등을 루트 모듈에 선언을 해야지만 사용 가능하다.
